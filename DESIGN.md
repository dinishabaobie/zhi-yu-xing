...
“知与行”把个人网站处理成一张会持续扩展的知识索引桌：冷银灰像档案纸面，墨色文字负责秩序，朱红负责定位关键行动与正在生长的内容。图像以接触印样的方式承担首要证据，大尺度中文排版和非对称编排让游戏、工作与知识共享一个世界，并在各自板块中继续展开。
...
首屏使用约 0.82:1.18 的非对称两栏，左侧将用户提供的个人宣言分为三行标题与三行说明，右侧是 16:9 深色写信插画；间距随视口在 34–84px 之间变化。首页板块区采用 12 栏索引：游戏跨 7 栏，工作占右侧 4 栏并下移，知识从第二栏延伸至倒数第二栏，以横向星空插画统摄逻辑学与科学专题。板块页首屏使用大标题与“当前收录”双栏，专题列表继续使用不对称 12 栏。写作区和关于区都使用不等分双栏，强调编辑节奏而非等宽卡片阵列。
...
首页项目条目以摄影或插画为主、说明为辅，不设置整卡背景。游戏与知识使用横向画幅，工作在桌面使用 4:5 竖幅；三者都整块链接至站内板块页。知识入口使用用户提供的蓝色星空插画，板块页仍以各自的逻辑学与科学专题摄影展开内容。图像在条目悬停或链接获得焦点时，以 600ms 强缓出曲线轻微放大到 1.025 并增加饱和度；专题外链的无障碍名称明确提示将在新窗口打开。
...
- **Don't** 把三个板块入口或板块内专题改成等宽、等高、等起点的通用卡片网格。
- **Don't** 给每个文本容器都加背景、圆角和阴影；开放版面与细线是主要结构。
- **Don't** 用等宽技术字体或单一职业视觉语言限制游戏、工作与知识的共同入口。
