...
首屏使用约 0.82:1.18 的非对称两栏，左侧将用户提供的个人宣言分为三行标题与三行说明，右侧是 3:2 四联摄影；间距随视口在 34–84px 之间变化。首页板块区采用 12 栏索引：游戏跨 7 栏，工作占右侧 4 栏并下移，知识从第二栏延伸至倒数第二栏，并以逻辑与科学双联摄影表达内部结构。板块页首屏使用大标题与“当前收录”双栏，专题列表继续使用不对称 12 栏。写作区和关于区都使用不等分双栏，强调编辑节奏而非等宽卡片阵列。
...
- **Responsive Images:** 首屏图像声明 1536×1024 的真实尺寸；游戏插画使用经过高质量网页优化的 2400×1221 版本，工作插画使用 2000×3013 版本，其余项目图像声明 735×480 的真实尺寸，并提供对应的 `sizes` 规则生成响应式 `srcset`。图像由 CSS 绝对定位到容器四边，以 100% 宽高和 `object-fit: cover` 填满既定画幅；工作插画在桌面使用 4:5 画幅与 35% 纵向焦点。
...
首页项目条目以摄影为主、说明为辅，不设置整卡背景。游戏使用横向画幅，工作在桌面使用 4:5 竖幅，知识使用逻辑与科学双联画幅；三者都整块链接至站内板块页。板块页继续以摄影展开当前专题，图像组件保留 735×480 的真实尺寸与响应式 `sizes`。图像在条目悬停或链接获得焦点时，以 600ms 强缓出曲线轻微放大到 1.025 并增加饱和度；专题外链的无障碍名称明确提示将在新窗口打开。
