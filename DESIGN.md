...
首屏使用约 0.82:1.18 的非对称两栏，左侧将用户提供的个人宣言分为三行标题与三行说明，右侧是 16:9 深色写信插画；间距随视口在 34–84px 之间变化。首页板块区采用 12 栏索引：游戏跨 7 栏，工作占右侧 4 栏并下移，知识从第二栏延伸至倒数第二栏，以横向星空插画统摄逻辑学与科学专题；社会热点横跨全栏，以真实的判断账本预览替代装饰图片。板块页首屏使用大标题与“当前收录”双栏，专题列表继续使用不对称 12 栏。逻辑学与科学专题以站内二级目录条连接主站导航和原有阅读世界，分别保留纸面论证手册与科学微缩胶片的完整交互。热点案例页使用 42:58 双栏，左侧粘性证据时间线，右侧依次展开判断、逻辑、群体机制与改判条件；原话中的关键句与右侧判断共享关系标识，悬停或键盘聚焦时以“旅途愉快”群像语法的 2px 朱红贝塞尔线连接，点击可锁定，多段原话支持同时连向同一判断。窄屏折为单列时隐藏关系线。写作区和关于区都使用不等分双栏，强调编辑节奏而非等宽卡片阵列。
