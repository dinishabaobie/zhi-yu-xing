...
“知与行”把个人网站处理成一张会持续扩展的知识索引桌：冷银灰像档案纸面，墨色文字负责秩序，朱红负责定位关键行动与正在生长的内容。图像以接触印样的方式承担首要证据，大尺度中文排版和非对称编排让游戏、工作、知识与社会热点共享一个世界，并在各自板块中继续展开。
...
- **Display**（560，响应式 32–56px，1.1）：首页首屏的多行个人宣言；移动端收束到 32–42px，以用户给出的句组稳定断行。
...
逻辑学与科学专题迁入时保留各自已经形成的精细阅读层级。逻辑学使用宋体与无衬线的对照，并以 9、10、11、14、15、18、23px 等微型标注和正文台阶组织论证手册；科学专题使用 0.52至0.92rem 的档案标注、1.12至1.78rem 的阅读标题和更大的流式章节标题组织时间轴。两套专题字体只在各自作用域内生效，不改变主站全局字体层级。
...
首屏使用约 0.82:1.18 的非对称两栏，左侧将用户提供的个人宣言分为三行标题与三行说明，右侧是 16:9 深色写信插画；间距随视口在 34–84px 之间变化。首页板块区采用 12 栏索引：游戏跨 7 栏，工作占右侧 4 栏并下移，知识从第二栏延伸至倒数第二栏，以横向星空插画统摄逻辑学与科学专题；社会热点横跨全栏，以真实的判断账本预览替代装饰图片。板块页首屏使用大标题与“当前收录”双栏，专题列表继续使用不对称 12 栏。逻辑学与科学专题以站内二级目录条连接主站导航和原有阅读世界，分别保留纸面论证手册与科学微缩胶片的完整交互。热点案例页将四个日期做成独立关系单元：左侧是当天完整原文，右侧是严格截止当日的深度解析；1440px 以上采用约 34:66 的主栏比例与右侧双列判断网格，让原文自然增高、分析缩短，减少两栏内容长度不同造成的空洞。较窄桌面维持单列判断流。关键原句与对应判断共享关系标识，悬停或键盘聚焦时以“旅途愉快”群像语法的 2px 朱红贝塞尔线连接，点击可锁定。每个日期都累计使用当日及此前全部材料，同时把分析封闭在自己的时间上限内；后续材料不得提前回写。窄屏折为单列时隐藏关系线。写作区和关于区都使用不等分双栏，强调编辑节奏而非等宽卡片阵列。
...
系统采用“平面为主、证据轻抬”的混合深度。大部分文字与结构线直接落在页面底色上；阴影只用于首屏图像、项目摄影和移动导航，半透明页眉通过 18px 模糊与 120% 饱和度形成覆盖层。首屏图像后方另有向左上错位的细线框，桌面页面边缘还有一像素内框，使深度更像叠放的档案纸而不是浮动卡片。

### Shadow Vocabulary

- **首屏浅色环境影**（`0 26px 70px rgb(43 49 45 / 13%)`）：用于首屏图像和移动导航。
...
- **Corner Style:** 项目摄影与首屏图像使用轻柔圆角（14px），项目说明本身不设封闭卡片。
- **Background:** 图片加载底使用压暗银灰；写作面板使用整块朱红；移动菜单使用雾白档案面。
- **Shadow Strategy:** 仅摄影和浮层使用 Elevation 章节中的环境影。
- **Border:** 项目说明以顶部一像素强线开始；首屏图像后方使用错位细线框。
- **Internal Padding:** 项目说明顶部 20px；写作面板为响应式 72–112px，移动端为上下 72px、左右 24px。
- **Responsive Images:** 首屏插画声明 2400×1350 的真实尺寸；游戏插画使用经过高质量网页优化的 2400×1221 版本，工作插画使用 2000×3013 版本，知识插画使用 2400×1244 版本，板块页专题图像声明 735×480 的真实尺寸，并提供对应的 `sizes` 规则生成响应式 `srcset`。图像由 CSS 绝对定位到容器四边，以 100% 宽高和 `object-fit: cover` 填满既定画幅；工作插画在桌面使用 4:5 画幅与 35% 纵向焦点。
...
首页项目条目以摄影、插画或真实内容预览为主、说明为辅，不设置整卡背景。游戏与知识使用横向画幅，工作在桌面使用 4:5 竖幅；三者都整块链接至站内板块页。知识板块中的逻辑学与科学使用站内链接和右箭头，游戏与工作保留外链并明确提示新窗口。社会热点使用深墨案件卷宗预览，直接展示当前命题及“接受、拒绝、待证”状态，不虚构新闻图片。图像在条目悬停或链接获得焦点时，以 600ms 强缓出曲线轻微放大到 1.025 并增加饱和度。

### Social Case Dossier

社会热点索引以案件标题、核心问题与当前判断作为入口。案例页顶部先给出暂定结论与证据状态；主体左侧将材料标记为原始发言、当事方说明或公司声明，右侧将每个争议命题分别归入接受、有条件接受、暂缓判断或拒绝。朱红只用于关键判断与焦点，状态不能只靠颜色表达。长文以开放排版、稀疏分隔线和有限的深墨证据面组织，不使用新闻卡片瀑布流。

“旭旭宝宝与《功夫女足》争议”采用逐日累计、向前封闭的分析：16 日只分析 16 日；18 日综合 16、18 日；19 日综合此前三日；20 日综合四日全部材料。后来澄清不能提前成为早期判断的前提。原文中的关键句和右栏分析节点使用同一关系标识，悬停、聚焦与点击时显示群像式曲线；节点额外标注有效判断、当事人错误、舆论升级点、证据边界或回应决策。
...
- **Do** 在桌面保持作品跨栏、画幅和纵向错位，并让热点页的证据与分析保持清楚的双栏关系。
- **Do** 为动态提供减少动态降级，为模糊页眉提供减少透明度降级。

### Don't:

- **Don't** 引入蓝色或其他竞争性强调色，破坏单一朱红系统。
- **Don't** 把四个板块入口或板块内专题改成等宽、等高、等起点的通用卡片网格。
- **Don't** 给每个文本容器都加背景、圆角和阴影；开放版面与细线是主要结构。
- **Don't** 用等宽技术字体或单一职业视觉语言限制游戏、工作、知识与社会热点的共同入口。
