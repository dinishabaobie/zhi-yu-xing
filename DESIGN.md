...
“旭旭宝宝与《功夫女足》争议”采用逐日封闭分析。四段完整原文按日期排列，每一天的右栏只使用截止当天已经出现的材料；后来澄清不能提前成为早期判断的前提。原文中的关键句和右栏分析节点使用同一关系标识，悬停、聚焦与点击时显示群像式曲线。
