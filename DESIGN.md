...
首屏使用约 0.82:1.18 的非对称两栏，左侧将用户提供的个人宣言分为三行标题与三行说明，右侧是 16:9 深色写信插画；间距随视口在 34–84px 之间变化。首页板块区采用 12 栏索引：游戏跨 7 栏，工作占右侧 4 栏并下移，知识从第二栏延伸至倒数第二栏，以横向星空插画统摄逻辑学与科学专题；社会热点横跨全栏，以真实的判断账本预览替代装饰图片。板块页首屏使用大标题与“当前收录”双栏，专题列表继续使用不对称 12 栏。热点案例页使用 42:58 双栏，左侧粘性证据时间线，右侧依次展开判断、逻辑、群体机制与改判条件。写作区和关于区都使用不等分双栏，强调编辑节奏而非等宽卡片阵列。
...
首页项目条目以摄影、插画或真实内容预览为主、说明为辅，不设置整卡背景。游戏与知识使用横向画幅，工作在桌面使用 4:5 竖幅；三者都整块链接至站内板块页。社会热点使用深墨案件卷宗预览，直接展示当前命题及“接受、拒绝、待证”状态，不虚构新闻图片。图像在条目悬停或链接获得焦点时，以 600ms 强缓出曲线轻微放大到 1.025 并增加饱和度；专题外链的无障碍名称明确提示将在新窗口打开。
