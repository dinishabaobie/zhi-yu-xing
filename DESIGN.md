...
“知与行”把个人网站处理成一张会持续扩展的知识索引桌：冷银灰像档案纸面，墨色文字负责秩序，朱红负责定位关键行动与正在生长的内容。摄影以接触印样的方式承担首要证据，大尺度中文排版和非对称编排让游戏、工作与知识共享一个世界，并在各自板块中继续展开。
...
首屏使用约 0.82:1.18 的非对称两栏，左侧将用户提供的个人宣言分为三行标题与三行说明，右侧是 3:2 四联摄影；间距随视口在 34–84px 之间变化。首页板块区采用 12 栏索引：游戏跨 7 栏，工作占右侧 4 栏并下移，知识从第二栏延伸至倒数第二栏，以横向星空插画统摄逻辑学与科学专题。板块页首屏使用大标题与“当前收录”双栏，专题列表继续使用不对称 12 栏。写作区和关于区都使用不等分双栏，强调编辑节奏而非等宽卡片阵列。
...
- **Responsive Images:** 首屏图像声明 1536×1024 的真实尺寸；游戏插画使用经过高质量网页优化的 2400×1221 版本，工作插画使用 2000×3013 版本，知识插画使用 2400×1244 版本，板块页专题图像声明 735×480 的真实尺寸，并提供对应的 `sizes` 规则生成响应式 `srcset`。图像由 CSS 绝对定位到容器四边，以 100% 宽高和 `object-fit: cover` 填满既定画幅；工作插画在桌面使用 4:5 画幅与 35% 纵向焦点。
